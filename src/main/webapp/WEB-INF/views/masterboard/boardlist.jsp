...
<%--[외부 파일] --%>
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/css/Main/main_dece_notice_notice.css">
	...
	<main>
		<div class="container mt-3">
		  ...
		  <table class="table table-hover">
				...
				<!-- 화면 출력 번호 -->
				<c:set var="board_num" value="${listcount-(page-1)*5}"/>
				<c:forEach var="b" items="${boardlist }">
					<tr>
						<td>${board_num}<c:set var="board_num"
								value="${board_num-1 }" />
						</td>
						<td>
					<a href="<%=request.getContextPath()%>/master/boardcontent.do?board_num=${b.board_num}&page=${page}">
								${b.board_title } </a></td>
						<td>${b.board_name }</td>
			...
			<center>
				<c:if test="${listcount > 0 }">
					...
					<!-- 1페이지로 이동 -->
						...
						<a href="<%=request.getContextPath()%>/master/main_notice.do?page=1"style="text-decoration: none"> << </a>
							

						<!-- 이전 블럭으로 이동 -->
						<c:if test="${startPage > 5 }">
							<a href="<%=request.getContextPath()%>/master/main_notice.do?page=${startPage-5}"> [이전] </a>
						</c:if>
						...
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							...
							<c:if test="${i != page }">
								<!-- 현재 페이지가 아닌 경우 -->
							<a href="<%=request.getContextPath()%>/master/main_notice.do?page=${i}">[${i}]</a>
							</c:if>
						</c:forEach>

						<!-- 다음 블럭으로 이동 -->
						<c:if test="${endPage < pageCount}">
						<a href="<%=request.getContextPath()%>/master/main_notice.do?page=${startPage+5}">[다음]</a>
						</c:if>

						<!-- 마지막 페이지로 이동 -->
						<a href="<%=request.getContextPath()%>/master/main_notice.do?page=${pageCount}"
